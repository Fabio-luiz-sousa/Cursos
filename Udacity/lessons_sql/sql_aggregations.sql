...
SELECT accounts.name, AVG(standard_qty) AS mean_standard_qty, AVG(gloss_qty) AS mean_gloss_qty, AVG(poster_qty) AS mean_poster_qty
FROM orders
JOIN accounts
...
SELECT accounts.name, AVG(standard_amt_usd) AS mean_standard_amt_usd, AVG(gloss_amt_usd) AS mean_gloss_amt_usd, AVG(poster_amt_usd) AS mean_poster_amt_usd
FROM orders
JOIN accounts
ON accounts.id=orders.account_id
GROUP BY accounts.name
...
the name of the sales rep, the channel, 
...
WHEN total_amt_usd < 3000 THEN 'Small' END AS level_order
FROM orders
...
the level associated with each account. You should provide the account name, the 
total sales of all orders for the customer, and the level. Order with the top spending customers listed first.*/
...
which are sales reps associated with more than 200 orders or more than 750000 in total sales. The middle group has any rep with more 
than 150 orders or 500000 in sales. Create a table with the sales rep name, the total number of orders, total sales across all orders, 
nd a column with top, middle, or low depending on this criteria. Place the top sales people based on dollar amount of sales first in your final table.*/
